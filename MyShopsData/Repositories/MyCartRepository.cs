...
{
    public class MyCartRepository : BaseRepository<MyCartDto>, IMyCartRepository
    {
        public MyCartRepository(IDatabaseFactory databaseFactory) : base(databaseFactory)
        {
        }
        ...
        public async Task<string> AddItem(MyCartDto item)
        {
            var parameters = new DynamicParameters();
        ...
        public async Task<string> UpdateItem(int userId, int id, int quantity)
        {
            ...
            var res = await DataContext.QueryAsync($"update [Users].[UsersCart] set   quantity = {quantity} where userId = {userId} and id = {id}");
            if(res != null)
            {
                return "success";
            }
